...
const isResolved = ( { absolutepaths, jsons } ) => absolutepaths.length + jsons.length === 0;
const resolve = ( { context } ) => context.promise.then( () => isResolved( context ) ? context.root :
	Promise.all( context.absolutepaths.splice( 0 ).map( absolutepath => modules.files( absolutepath, '*.proto' ) ) )
...
class ProtoParser {
	...
	json( ...args ) { this.promise = resolve( this ).then( root => root.toJSON().nested ).then( ...args ); return this; }
	get add() { return {
		path: ( scanpath = '.' ) => {
